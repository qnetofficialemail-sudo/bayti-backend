...
# ── 1. Create routers/studio.py ──────────────────────────────────────
studio_router = '''from fastapi import APIRouter, Depends, HTTPException, UploadFile, File, Form
...
# Write studio router
with open(os.path.join(BACKEND, "routers", "studio.py"), "w", encoding="utf-8") as f:
    f.write(studio_router)
print("✅ Created routers/studio.py")

# ── 2. Register router in main.py ─────────────────────────────────────
main_path = os.path.join(BACKEND, "main.py")
main_content = open(main_path, encoding="utf-8").read()

if "studio" not in main_content:
    # Add import
    old_import = "from routers import auth, products, sellers, orders, admin, ai"
    new_import = "from routers import auth, products, sellers, orders, admin, ai, studio"
    if old_import in main_content:
        main_content = main_content.replace(old_import, new_import)
        print("✅ Added studio import")
    else:
        # Try to find any router import line
        lines = main_content.split("\n")
        for i, line in enumerate(lines):
            if "from routers import" in line:
                lines[i] = line.rstrip() + ", studio"
                main_content = "\n".join(lines)
                print(f"✅ Added studio import to: {line.strip()}")
                break

    # Add router include
    old_include = "app.include_router(ai.router)"
    new_include = "app.include_router(ai.router)\napp.include_router(studio.router)"
    if old_include in main_content:
        main_content = main_content.replace(old_include, new_include)
        print("✅ Added studio router include")
    else:
        print("⚠️  Could not find ai.router include — add manually: app.include_router(studio.router)")

    with open(main_path, "w", encoding="utf-8") as f:
        f.write(main_content)
else:
    print("✅ Studio already in main.py")

print("\nDone! Now:")
print("  git add -A")
print('  git commit -m "Add Bayti Smart Studio — AI product photography endpoint"')
print("  git push")
