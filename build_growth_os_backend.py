...
# ── 1. Create the outreach router ─────────────────────────────────────
outreach_router = '''from fastapi import APIRouter, Depends, HTTPException
...
# Write router
router_path = os.path.join(BACKEND, "routers", "growth.py")
with open(router_path, "w", encoding="utf-8") as f:
    f.write(outreach_router)
print("✅ Created routers/growth.py")

# Register in main.py
main_path = os.path.join(BACKEND, "main.py")
main_content = open(main_path, encoding="utf-8").read()

if "growth" not in main_content:
    # Add import
    for line in main_content.split("\n"):
        if "from routers import" in line:
            old_import = line
            new_import = line.rstrip() + ", growth"
            main_content = main_content.replace(old_import, new_import)
            print("✅ Added growth import")
            break
    # Add router
    old_include = "app.include_router(studio.router)"
    new_include = "app.include_router(studio.router)\napp.include_router(growth.router)"
    if old_include in main_content:
        main_content = main_content.replace(old_include, new_include)
        print("✅ Added growth router")
    with open(main_path, "w", encoding="utf-8") as f:
        f.write(main_content)
else:
    print("✅ Growth already in main.py")
