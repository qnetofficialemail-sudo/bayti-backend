...
INITIAL_ACCOUNTS = [
    ...
    {"username": "scentlibraryofficial","display_name":"Scent Library",     "category": "عطور وهدايا",       "emirate": "دبي",     "product_note": "دار عطور في الإمارات"},
    ...
    {"username": "apple_wenz_abaya",  "display_name": "Apple Wenz Abaya",  "category": "عبايات منزلية",     "emirate": "العين",   "product_note": "عبايات ومخاوير مع توصيل"},
]
...
def generate_message(data: dict, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    ...
    response = client.messages.create(
        ...
        max_tokens=600,
        system="""أنت مسؤول التواصل في فريق بيتي — منصة محلية في الإمارات تجمع البائعين والمشترين من جميع الجنسيات.
...
- البائعون متنوعون: من يصنع يدوياً، من يطبخ، ومن يستورد ويعيد البيع
- لا تقتصر على المنتجات اليدوية فقط
